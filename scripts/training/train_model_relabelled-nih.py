...
parser.add_argument('-name', type=str, default="cropped_relabel-nih")
parser.add_argument('--output_dir',
                    type=str,
                    default="./output/output_all_cropped_relabelled-nih/")
parser.add_argument('--dataset',
                    type=str,
                    default="chex-nih-mimic_ch-pc-google-openi-kaggle")
...
"""
transforms = torchvision.transforms.Compose([xrv.datasets.XRayResizer(224)])
...
if "densenet" in cfg.model:
    model = xrv.models.DenseNet(num_classes=train_dataset.labels.shape[1],
                                in_channels=1,
                                **xrv.models.get_densenet_params(cfg.model))
elif "resnet101" in cfg.model:
    model = torchvision.models.resnet101(
        num_classes=train_dataset.labels.shape[1], pretrained=False)
    ...
    model = torchvision.models.shufflenet_v2_x2_0(
        num_classes=train_dataset.labels.shape[1], pretrained=False)
    #patch for single channel
    model.conv1[0] = torch.nn.Conv2d(1,
                                     24,
                                     kernel_size=3,
                                     stride=2,
                                     padding=1,
