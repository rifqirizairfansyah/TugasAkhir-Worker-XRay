...
class COVID19_Indonesia_Dataset(Dataset):
    """
    Johnson AE, Pollard TJ, Berkowitz S, Greenbaum NR, Lungren MP, Deng CY, Mark RG, Horng S. MIMIC-CXR: A large publicly available database of labeled chest radiographs. arXiv preprint arXiv:1901.07042. 2019 Jan 21.
    ...
    def __init__(self,
        ...
        super(COVID19_Indonesia_Dataset, self).__init__()
        np.random.seed(seed)  # Reset the seed so all runs are the same.
        self.MAXVAL = 255

        self.pathologies = ["ggo", "consolidation"]
        ...
        #self.csv = self.csv.set_index(['id', 'study_id'])
        #self.csv = self.csv.join(self.metacsv).reset_index()
        ...
        idx_pa = self.csv["view"].isin(views)
        self.csv = self.csv[idx_pa]

        if unique_patients:
            self.csv = self.csv.groupby("id").first().reset_index()
    ...
    def __getitem__(self, idx):
        ...
        source = str(self.csv.iloc[idx]["source"])
        status = str(self.csv.iloc[idx]["status"])
        patient_id = str(self.csv.iloc[idx]["id"])
        filename = str(self.csv.iloc[idx]["filename"])

        img_path = os.path.join(self.imgpath, source, status, patient_id,
                                filename)
        img = imread(img_path)
        img = normalize(img, self.MAXVAL)
...
class COVID19_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        # defined here to make the code easier to readfile
        pneumonias = [
            "COVID-19", "SARS", "MERS", "ARDS", "Streptococcus",
            "Pneumocystis", "Klebsiella", "Chlamydophila", "Legionella",
            "Lipoid", "Influenza", "Mycoplasma", "Varicella"
        ]

        self.pathologies = [
            "Pneumonia", "Viral Pneumonia", "Bacterial Pneumonia",
            "Fungal Pneumonia", "No Finding"
        ] + pneumonias
        self.pathologies = sorted(self.pathologies)

        mapping = dict()
        mapping["Pneumonia"] = pneumonias
        mapping["Viral Pneumonia"] = [
            "COVID-19", "SARS", "MERS", "Influenza", "Varicella"
        ]
        mapping["Bacterial Pneumonia"] = [
            "Streptococcus", "Klebsiella", "Chlamydophila", "Legionella",
            "Mycoplasma"
        ]
